...
def group_into_umis(seqID, aUMI_dict):
    if seqID in aUMI_dict.keys():
        return aUMI_dict[seqID]
    else:
        return 0


def group_into_umis_unpaired(seqID, ):
    if seqID in aUMI_unpaired.keys():
...
def queu_group_umi_seq(seq, out_dir, aUMI_dict, threads=1, paired=1):
    '''基于umi将序列分组（多进程）
    参数：
        seq: 输入序列文件（fastq）
        out_dir: 分组序列输出目录
    ...
    with eval(file_open)(seq, 'rt') as seq_handle:
        ...
        umi2seq_tmp = []
        for seq_tmp in seqs:
            uID = eval(func)(seq_tmp, aUMI_dict)
            count_seq += 1
            if uID:
                total_umi_seq_dict[uID] = total_umi_seq_dict.get(uID, [])
                total_umi_seq_dict[uID].append(seq_tmp)
                total_check_seq += 1
                umi_counts_sub[uID] = umi_counts_sub.get(uID, 0) + 1
