...
#     sys.exit()
    ...
    A_umi_count_file = os.path.join(out_dir, File_Tag + '_A_umi_dropRepeat_count.csv')
    A_uID_file = os.path.join(out_dir, File_Tag+'_A_uID_dropRepeat.csv')
    A_uID_count_file = os.path.join(out_dir, File_Tag + '_A_uID_dropRepeat_count.csv')
...
#     A2_umi_file_info = A2_umi_file + '.cutadapt.info.file'
    ...
    try:
        ...
        df_Aumi_count.to_csv(A_umi_count_file, index=False)
        df_AuID_count.to_csv(A_uID_count_file, index=False)
    except Exception as e:
        logging.info(e)
